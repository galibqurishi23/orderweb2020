...
function OrderSummary({
  ...
  React.useEffect(() => {
    if (selectedOrderType === 'delivery' || (selectedOrderType === 'advance' && advanceFulfillmentType === 'delivery')) {
      // Calculate delivery fee based on postcode - simplified for now
      setDeliveryFee(2.99); // Default delivery fee
    } else {
      setDeliveryFee(0);
    }
  }, [selectedOrderType, advanceFulfillmentType, postcode]);
  ...
  const voucherDiscount = appliedVoucher ? Math.min(10, subtotal) : 0; // Simplified voucher discount
  ...
  const handleApplyVoucher = () => {
    ...
    // Simplified voucher validation
    if (voucherInput.toLowerCase() === 'save10') {
      setAppliedVoucher({ code: 'SAVE10' } as Voucher);
      setVoucherError('');
      toast({
        title: 'Voucher Applied!',
        description: `You saved ${currencySymbol}10.00`,
      });
    } else {
